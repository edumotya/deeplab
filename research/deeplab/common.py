...
flags.DEFINE_boolean(
    "keep_aspect_ratio", True, "Keep aspect ratio after resizing or not."
)
...
flags.DEFINE_bool(
    "nas_remove_os32_stride", False, "Remove the stride in the output stride 32 branch."
)
...
flags.DEFINE_boolean(
    "aspp_with_concat_projection", True, "ASPP with concat projection."
)

flags.DEFINE_boolean(
    "aspp_with_squeeze_and_excitation", False, "ASPP with squeeze and excitation."
)
...
flags.DEFINE_boolean(
    "decoder_output_is_logits", False, "Use decoder output as logits or not."
)
    ...
    """
        ...
        if FLAGS.decoder_output_stride:
            ...
            if sorted(decoder_output_stride, reverse=True) != decoder_output_stride:
                raise ValueError(
                    "Decoder output stride need to be sorted in the "
                    "descending order."
                )
