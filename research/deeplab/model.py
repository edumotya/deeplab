...
def predict_labels_multi_scale(
    images, model_options, eval_scales=(1.0,), add_flipped_images=False
):
  ...
  """
    outputs_to_predictions = {
        output: [] for output in model_options.outputs_to_num_classes
    }
...
def multi_scale_logits(
  ...
  """
    ...
    if not image_pyramid:
        image_pyramid = [1.0]
    crop_height = (
        model_options.crop_size[0] if model_options.crop_size else tf.shape(images)[1]
    )
    crop_width = (
        model_options.crop_size[1] if model_options.crop_size else tf.shape(images)[2]
    )
    ...
    logits_height = scale_dimension(
        crop_height, max(1.0, max(image_pyramid)) / logits_output_stride
    )
    logits_width = scale_dimension(
        crop_width, max(1.0, max(image_pyramid)) / logits_output_stride
    )
    ...
    for output in sorted(model_options.outputs_to_num_classes):
        ...
        merge_fn = (
            tf.reduce_max if model_options.merge_method == "max" else tf.reduce_mean
        )
        ...
        if model_options.dense_prediction_cell_config is not None:
            ...
            with slim.arg_scope(
                ...
                with slim.arg_scope([batch_norm], **batch_norm_params):
                    ...
                    if model_options.add_image_level_feature:
                        if model_options.crop_size is not None:
                            image_pooling_crop_size = (
                                model_options.image_pooling_crop_size
                            )
                    ...
                    if (
                        model_options.add_image_level_feature
                        and model_options.aspp_with_squeeze_and_excitation
                    ):
                ...
                for output_stride in decoder_output_stride:
                    ...
                    for i, name in enumerate(feature_list):
                        ...
                        for j, feature in enumerate(decoder_features_list):
                            ...
                            h = (
                                None
                                if isinstance(decoder_height, tf.Tensor)
                                else decoder_height
                            )
                            w = (
                                None
                                if isinstance(decoder_width, tf.Tensor)
                                else decoder_width
                            )
...
def _decoder_with_concat_merge(
    decoder_features_list,
    decoder_depth,
    decoder_use_separable_conv=True,
    weight_decay=0.0001,
